...
void heap_bubble_down(heap_t *heap, int index) {
    int child = heap_left_child(index);
    int child2 = heap_right_child(index);

    if (child < heap_size(heap)) {
        if (child2 < heap_size(heap) && heap->data[child2].key < heap->data[child].key) {
            child = child2;
        }

        if (heap->data[child].key < heap->data[index].key) {
